...
const searchAndNavigate = () => {
  const categoryToSearch = searchInput.value.toLowerCase();
  const categoryExists = categories.some(category => category.title.toLowerCase() === categoryToSearch);

  if (categoryExists) {
    window.location.href = `searchresult.html?category=${encodeURIComponent(categoryToSearch)}`;
  } else {
...
searchButton.addEventListener('mousedown', searchAndNavigate);

// Handle search when the user presses Enter in the search input
